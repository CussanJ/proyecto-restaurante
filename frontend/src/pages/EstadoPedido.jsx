...
export default function EstadoPedido() {
  ...
  const cancelarPedidoCliente = async () => {
        ...
        <section className="bg-surface-container rounded-2xl p-6 border border-neutral-800">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h1 className={`text-2xl font-bold ${esCancelado ? 'text-red-500' : 'text-on-surface'}`}>
                {esCancelado ? 'Pedido cancelado' : TITULO[estadoIdx]}
              </h1>
            </div>
            {esCancelado ? (
          ...
          {esCancelado && (
            <div className="mt-4 pt-4 border-t border-neutral-800 flex items-center gap-3">
              ...
              <p className="text-sm text-neutral-400">Este pedido ha sido cancelado y no será procesado.</p>
            </div>
          )}
        </section>
          ...
          <div className="p-5 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 mt-2">
              {(pedido.estado === 'pendiente' || pedido.estado === 'aceptado') && (
                <button
                  onClick={cancelarPedidoCliente}
                  className="w-full bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors active:scale-95"
                >
